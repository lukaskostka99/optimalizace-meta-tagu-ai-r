...
Tento Quarto Document vznikl jako ukázka kódu pro práci s OpenAI v R v rámci konference SEO Restart. Jedná se pouze o příklad a modifikace je možná pro konkrétní příklady. V případě dorazů mě můžete kontaktovat na lukas.kostka\@effectix.com.
...
scdata <- search_analytics(
  ...
  dimensionFilterExp = c("country==CZE", "query!~brand_vylouceni"),
  rowLimit = 20000
  )
```
...
Výběr počtu URL, které chci optimalizovat
...
for (i in 1:nrow(url_data)) {
  ...
  # Použijte xpath k vybrání textu z webové stránky
  text <- web_stranka  |> 
    html_nodes('p')  |> 
...
```{r}
Sys.setenv(OPENAI_API_KEY = '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')
web <-  "Gutta"
